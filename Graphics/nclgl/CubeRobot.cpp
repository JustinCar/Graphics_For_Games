...
CubeRobot::CubeRobot(void) {
	...
	SceneNode* hips = new SceneNode(cube, Vector4(1, 1, 0, 1)); // Yellow !
	hips->SetModelScale(Vector3(10, 3, 5));
	hips->SetTransform(Matrix4::Translation(Vector3(0, 35, 0)));
	AddChild(hips);

	SceneNode * body = new SceneNode(cube, Vector4(1, 0, 0, 1)); // Red !
	body -> SetModelScale(Vector3(10, 15, 5));
	body -> SetTransform(Matrix4::Translation(Vector3(0, 6, 0)));
	hips->AddChild(body);
